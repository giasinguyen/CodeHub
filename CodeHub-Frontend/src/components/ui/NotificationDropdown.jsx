...
const NotificationDropdown = ({
  ...
  const loadNotifications = useCallback(async () => {
    ...
    try {
      console.log(
        "🔔 [NotificationDropdown] Loading notifications from API..."
      );
      ...
      if (response && response.data && response.data.content) {
        ...
        const transformedNotifications = response.data.content.map(
          (notification) => ({
            id: notification.id,
        ...
        console.log(
          "✅ [NotificationDropdown] Loaded notifications from API:",
          transformedNotifications.length
        );
      } else {
        console.log(
          "⚠️ [NotificationDropdown] Empty API response, using mock data"
        );
        setNotifications(getMockNotifications());
      }
    } catch (error) {
      console.warn(
        "⚠️ [NotificationDropdown] API not available, using mock data:",
        error.message
      );
  ...
  const markAsRead = async (id) => {
    try {
      ...
      console.warn(
        "API error for marking notification as read:",
        error.message
      );
  ...
  const getNotificationIcon = (notification) => {
    ...
    return (
      <div
        className={`p-2 rounded-full bg-slate-700 dark:bg-slate-700 light:bg-gray-100 ${notification.iconColor}`}
      >
        ...
        >
          ...
          <div className="max-h-80 overflow-y-auto">
            {loading ? (
              ...
              <div className="divide-y divide-slate-700 dark:divide-slate-700 light:divide-gray-200">
                {" "}
