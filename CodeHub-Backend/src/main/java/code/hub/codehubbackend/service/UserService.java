...
public class UserService {
    ...
    public void followUser(Long userId) {
        ...
        // For now, just create an activity (following relationship would need a separate table)
    ...
    private Integer calculateExperienceLevel(Long snippetCount, Double reputation) {
        if (snippetCount == null || reputation == null) return 1;
        
        if (snippetCount >= 50 && reputation >= 80) return 5; // Expert
        if (snippetCount >= 25 && reputation >= 60) return 4; // Advanced
        if (snippetCount >= 10 && reputation >= 40) return 3; // Intermediate
        if (snippetCount >= 3 && reputation >= 20) return 2; // Beginner
        return 1; // Novice
    }
